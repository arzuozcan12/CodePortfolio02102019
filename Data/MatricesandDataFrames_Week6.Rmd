---
title: "Matrices and Dataframes - Week 6"
author: "Arzu Ozcan"
...
```
with(bsale, mean(price[color == "green"]))              # Calculates the mean
with(bsale, name[age > 100])                            # Finds the names of boats older than 100 years
with(subset(bsale, color == "black"), mean(profit > 0)) # Find The percent of black boats had a positive profit
bsale.2 <- bsale[c("price", "cost")]                    # Save only the price and cost columns in a new dataframe
names(bsale.2) <- c("p", "c")                           # Change the names of the columns to "p" and "c"
old.black.bsale <- subset(bsale, color == "black" & age > 50) # Create a dataframe called old.black.bsale containing only data from black boats older than 50 years
```
...
head(ToothGrowth[c("len", "supp")]) # Gives the len AND supp columns of ToothGrowth
...
### Gives the rows 1-6 and column 1 of ToothGrowth
ToothGrowth[1:6, 1]
### [1]  4.2 11.5  7.3  5.8  6.4 10.0

### Gives the rows 1-3 and columns 1 and 3 of ToothGrowth
ToothGrowth[1:3, c(1,3)]
####    len dose
#### 1  4.2  0.5
#### 2 11.5  0.5

### Gives the 1st row (and all columns) of ToothGrowth
ToothGrowth[1, ]
###   len supp dose len.cm index
### 1 4.2   VC  0.5   0.42     1

### Gives the 2nd column (and all rows) of ToothGrowth
ToothGrowth[, 2]
...
### Creates a new df with only the rows of ToothGrowth where supp equals VC
ToothGrowth.VC <- ToothGrowth[ToothGrowth$supp == "VC", ]

### Creates a new df with only the rows of ToothGrowth where supp equals OJ and dose < 1

ToothGrowth.OJ.a <- ToothGrowth[ToothGrowth$supp == "OJ" &
                                  ToothGrowth$dose < 1, ]

### Gets rows of ToothGrowth where len < 20 AND supp == "OJ" AND dose >= 1
...
### Gets rows of ToothGrowth where len > 30 AND supp == "VC", but only return the len and dose columns
subset(x = ToothGrowth,
